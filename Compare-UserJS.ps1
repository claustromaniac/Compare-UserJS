...
if ($fileNameA -ceq $fileNameB) { $fileNameA = $filepath_A; $fileNameB = $filepath_B }

# Used for padding the output in a few lists.
if ($fileNameA.length -ge $fileNameB.length) { $fn_pad = $fileNameA.length }
...
Function Write-Report {
	...
	$sep = (JSCom) + "------------------------------------------------------------------------------" + $nl
	if ($matching_prefs -and !($hideMask -band 1)) {$sep + (JSCom) + " The following " + $matches_count + " prefs match in both values and states:" + $nl+$nl + $matching_prefs + $nl}
	if ($differences -and !($hideMask -band 2)) {$sep + (JSCom) + " The following " + $diffs_count + " prefs have different values, but matching states:" + $nl+$nl + $differences + $nl}
	if ($missing_in_A -and !($hideMask -band 4)) {$sep + (JSCom) + " The following " + $missing_A_count + " prefs are not declared in " + $fileNameA + ":" + $nl+$nl + $missing_in_A + $nl}
	if ($missing_in_B -and !($hideMask -band 8)) {$sep + (JSCom) + " The following " + $missing_B_count + " prefs are not declared in " + $fileNameB + ":" + $nl+$nl + $missing_in_B + $nl}
	if ($inactive_in_A -and !($hideMask -band 16)) {$sep + (JSCom) + " The following " + $inactive_A_count + " prefs match but are inactive in " + $fileNameA + ":" + $nl+$nl + $inactive_in_A + $nl}
	if ($inactive_in_B -and !($hideMask -band 32)) {$sep + (JSCom) + " The following " + $inactive_B_count + " prefs match but are inactive in " + $fileNameB + ":" + $nl+$nl + $inactive_in_B + $nl}
	if ($fully_mismatching -and !($hideMask -band 64)) {$sep + (JSCom) + " The following " + $fm_count + " prefs have both mismatching values and states:" + $nl+$nl + $fully_mismatching + $nl}
	if ($bad_syntax_A -and !($hideMask -band 128)) {$sep + (JSCom) + " " + $errors_A_count + " possible syntax errors detected in " + $fileNameA + ":" + $nl+$nl + $bad_syntax_A + $nl}
	if ($bad_syntax_B -and !($hideMask -band 128)) {$sep + (JSCom) + " " + $errors_B_count + " possible syntax errors detected in " + $fileNameB + ":" + $nl+$nl + $bad_syntax_B + $nl}
}
...
if ($append) { Write-Report | Out-File $outputFile -append }
